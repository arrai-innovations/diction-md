...
const SENTENCE_BOUNDARY =
  /(?<=[.!?])(?<!\b(?:[eE]\.g|[iI]\.e|[vV]s|[cC]f)\.)\s+(?=["'(]?[A-Z0-9])/g;
...
export const DEFAULT_WORDING_RULES = [
  ...
  },
  {
    category: "idiom",
    pattern:
      /\b(?:reach for|dive into|kick off|circle back|drill down|by a beat)\b/gi,
    message:
      "Consider literal wording for readers who use English as an additional language.",
  },
];
...
export function extractProseBlocks(source) {
  ...
  for (const [index, originalLine] of lines.entries()) {
    ...
    if (
      trimmed.startsWith("|") ||
      (trimmed.includes("|") &&
        TABLE_DELIMITER.test((lines[index + 1] ?? "").trim()))
    ) {
      flush();
      inTable = true;
      continue;
...
export function lintMarkdown(source, optionOverrides = {}) {
  ...
  for (const block of blocks) {
    ...
    if (block.includeInMetrics) {
      for (const [index, sentence] of blockSentences.entries()) {
        ...
        const displaySentence = aligned
          ? displaySentences[index]
          : block.display;
        ...
        if (words >= options.hardSentenceWords) {
          findings.push({
            severity: "warning",
            category:
              words >= options.veryHardSentenceWords
                ? "very-long-sentence"
                : "long-sentence",
            line,
      ...
      if (
        ...
        blockSentences.length > options.longParagraphSentences
      ) {
        findings.push({
          severity: "warning",
    ...
    if (options.prohibitDashes) {
      ...
      if (dashes.length) {
        findings.push({
          ...
          message:
            "Replace em dashes and en dashes with other punctuation or wording.",
          text: block.display,
        });
      }
    }
  ...
  const words = sentences.flatMap(
    (sentence) => sentence.match(WORD_PATTERN) || [],
  );
